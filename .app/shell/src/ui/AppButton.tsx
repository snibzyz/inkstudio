...
export type AppButtonColor = 'primary' | 'accent' | 'violet' | 'emerald' | 'sky' | 'pink' | 'zinc'
...
// ขนาด INKIDEA dense (settings/toolbar): h-8 px-3 text-[12px] — VS Code action button
// หากต้องการ workflow size ใหญ่กว่า ให้ override: className="!h-10 !px-4 !text-[13px]"
const BASE =
  'inline-flex items-center justify-center gap-1.5 rounded-sm border h-8 px-3 text-[12px] font-medium transition-colors focus:outline-none focus-visible:ring-1 focus-visible:ring-vscode-brand/40 disabled:cursor-not-allowed disabled:opacity-45'

const SOLID: Record<AppButtonColor, string> = {
  // primary = INKREALM brand amber (#F59E0B) — ใช้กับ action หลักของแอป
  primary: 'border-transparent bg-vscode-brand text-white hover:bg-vscode-brand-hover',
  // accent = VS Code blue (#0e639c) — สำหรับ chrome/IDE-ish action
  accent:  'border-transparent bg-vscode-accent text-white hover:bg-vscode-accent-hover',
...
const FLAT: Record<AppButtonColor, string> = {
  primary: 'border-vscode-brand/55 bg-vscode-brand/18 text-[#fde6c0] hover:bg-vscode-brand/26',
  accent:  'border-vscode-focus/55 bg-vscode-focus/18 text-[#d8ecff] hover:bg-vscode-focus/26',
...
export const AppButton = forwardRef<HTMLButtonElement, AppButtonProps>(function AppButton(
  ...
  if (variant === 'icon') {
    toneClass = 'border-transparent bg-transparent !h-8 !w-8 !p-0 text-vscode-muted hover:bg-vscode-list-hover hover:text-vscode-fg'
  } else if (variant === 'chrome') {
    toneClass = tone === 'primary'
      ? 'border-vscode-brand bg-vscode-brand text-white hover:bg-vscode-brand-hover'
